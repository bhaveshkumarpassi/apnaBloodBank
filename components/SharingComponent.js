...
import { View, Image, ImageBackground, Text, FlatList, ScrollView, SafeAreaView, ActivityIndicator, Dimensions, RefreshControl} from 'react-native';
import {ListItem, Card, Avatar, Button, Tile, Icon, Input} from 'react-native-elements';
import {connect} from 'react-redux';
import {Loading} from './LoadingComponent';
import Modal from 'react-native-modal';
import { auth } from '../firebase/firebase';
import {normalize} from '../assets/fonts/DynamicFontSize';
import { commentPost, commentRemove, fetchComments, fetchLikes, fetchPosts, likePost, likeRemove } from '../redux/ActionCreators';
...
const mapDispatchToProps = (dispatch) => {
    ...
    return {
        ...
        likeRemove: (likeId) => dispatch(likeRemove(likeId)),
        commentPost: (postId, comment) => dispatch(commentPost(postId, comment)),
        commentRemove: (commentId) => dispatch(commentRemove(commentId)),
...
class Sharing extends Component {
    ...
    onLike = async (postId) => {
        ...
        if(this.props.likes.likes.length && this.props.likes.likes.filter(like => (like.postId === postId && like.userId === this.state.user.uid))[0]) {
            
            var lik = this.props.likes.likes.filter(like => (like.postId === postId && like.userId === this.state.user.uid))[0];
            await this.props.likeRemove(lik._id);
        }
        else {
    ...
    onCommentSubmission = async () => {

        await this.props.commentPost(this.state.selectedPost, this.state.comment);
        ...
        if(this.state.isAuthenticated) {
                   ...
                   return (
                    ...
                    <ListItem.Content>
                        ...
                        <Icon
                            ...
                            color='red'
                            size={20}
                            name={ilike ? 'heart' : 'heart-o'}
                            onPress={() => this.onLike(item._id)}
                        ...
                        <Icon
                            ...
                            color='#512DA8'
                            size={20}
                            onPress={() => this.toggleWriteModal(item._id)}
                        />
                        <Icon
                                raised
                                reverse
            ...
            return (
                ...
                <SafeAreaView style={{flex: 1}}>
                        <FlatList
                            data={this.props.posts.posts.sort((a, b) => b.dateTime-a.dateTime)}
                ...
                <View style={{ alignItems: 'center', justifyContent: 'center', backgroundColor: '#519872', flexDirection: 'row'}}>
                    <Icon       
                        raised
                        reverse
                        name={'plus'}
