...
class OMinusDonorList extends Component {
    ...
    componentDidMount() {
        ...
        this.setState({
            data: this.props.users.users.filter((user) => user.bloodgroup === 'O-' && user.uid !== auth.currentUser.uid)
        })

        this.arrayHolder = this.props.users.users.filter((user) => user.bloodgroup === 'O-' && user.uid !== auth.currentUser.uid) ;
    }
...
export default connect(mapStateToProps)(OMinusDonorList);